...
// Tauri command to list available serial ports
#[tauri::command]
async fn play_alarm_sound_effect() -> () {
    let (_stream, handle) = rodio::OutputStream::try_default().unwrap();
    let sink = rodio::Sink::try_new(&handle).unwrap();

    let file: std::fs::File = std::fs::File::open("assets/alarm-digital-clock-beep.wav").unwrap();
    let source = rodio::Decoder::new(BufReader::new(file)).unwrap();
    let loop_source = source.take_duration(Duration::from_secs_f32(0.8));
